...
def firstRun():
	class MyWindow(xbmcgui.WindowXMLDialog):
		def __init__(self, *args, **kwargs):
			...
			self.msg   = "                         Bienvenido/a al instalador %s\nPulse en 'Wizard Menu' para instalar un build configurado por \n%s\n\n\n\n\n\nGracias por elegir %s" % (ADDONTITLE,BUILDERNAME, ADDONTITLE)
		...
		def doBuildMenu(self):
			wiz.log("[Check Updates] [User Selected: Wizard Menu] [Next Check: %s]" % str(NEXTCHECK), xbmc.LOGNOTICE)
			wiz.setS('lastbuildcheck', str(NEXTCHECK))
			self.close()
			url = 'plugin://%s/?mode=BuildList' % ADDON_ID
...
def updateWindow(name='Testing Window', current='1.0', new='1.1', icon=ICON, fanart=FANART):
	class MyWindow(xbmcgui.WindowXMLDialog):
		def __init__(self, *args, **kwargs):
			...
			self.fanart = kwargs['fanart']
			self.msgupdate  = "Actualizacion disponible para el build instalado:\n[COLOR %s]%s[/COLOR]\n\nActual Version: v[COLOR %s]%s[/COLOR]\nUltima Version: v[COLOR %s]%s[/COLOR]\n\n[COLOR %s]*Seleccione: Build o Parche[/COLOR]" % (COLOR1, self.name, COLOR1, self.current, COLOR1, self.new, COLOR4)
			self.msgcurrent = "Ejecutando ultima version del build instalado:\n[COLOR %s]%s[/COLOR]\n\nActual Version: v[COLOR %s]%s[/COLOR]\nUltima Version: v[COLOR %s]%s[/COLOR]\n\n[COLOR %s]*Seleccione: Build o Parche[/COLOR]" % (COLOR1, self.name, COLOR1, self.current, COLOR1, self.new, COLOR4)
