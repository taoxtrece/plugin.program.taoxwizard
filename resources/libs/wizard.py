...
def getCacheSize():
	...
	dbfiles   = [
		(os.path.join(ADDOND, 'plugin.video.elementum', 'cache.db', 'cache-backup.db')),
		(os.path.join(ADDOND, 'plugin.video.alfa', 'alfa_db.sqlite', 'cookies.dat')),
		(os.path.join(ADDOND, 'plugin.video.balandro', 'tmdb.sqlite', 'cookies.dat'))]
	cachelist = [
		(ADDOND),
	...
	if INCLUDEVIDEO == 'true':
		...
		if INCLUDEALL == 'true': files = dbfiles
			...
			if INCLUDEELEMENTUM == 'true':    files.append(os.path.join(ADDOND, 'plugin.video.elementum', 'cache.db', 'cache-backup.db'))
			if INCLUDEALFA == 'true':    files.append(os.path.join(ADDOND, 'plugin.video.alfa', 'alfa_db.sqlite', 'cookies.dat'))
			if INCLUDEBALANDRO == 'true':  files.append(os.path.join(ADDOND, 'plugin.video.balandro', 'tmdb.sqlite', 'cookies.dat'))
		if len(files) > 0:
...
def cleanupBackup():
	...
	selected = DIALOG.select("%s: Selecciona archivos para eliminar de 'MyBuilds'." % ADDONTITLE, list)
	
	if selected == -1:
		LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]Limpieza Cancelada![/COLOR]" % COLOR2)
	elif selected == 0: 
		if DIALOG.yesno(ADDONTITLE, "[COLOR %s]Quieres eliminar todos los archivos en la carpeta 'My_Builds'?[/COLOR]" % COLOR2, "[COLOR %s]%s[/COLOR]" % (COLOR1, MYBUILDS), yeslabel="[B][COLOR green]Limpiar[/COLOR][/B]", nolabel="[B][COLOR red]Cancelar[/COLOR][/B]"):
		...
		if DIALOG.yesno(ADDONTITLE, "[COLOR %s]Quieres eliminar [COLOR %s]%s[/COLOR] de la carpeta 'My_Builds'?[/COLOR]" % (COLOR2, COLOR1, list[selected]), "[COLOR %s]%s[/COLOR]" % (COLOR1, path), yeslabel="[B][COLOR green]Limpiar[/COLOR][/B]", nolabel="[B][COLOR red]Cancelar[/COLOR][/B]"):
...
def convertSpecial(url, over=False):
	...
	DP.create(ADDONTITLE, "[COLOR %s]Changing Physical Paths To Special" % COLOR2, "", "Espere...[/COLOR]")
	for root, dirs, files in os.walk(url):
		for file in files:
			start += 1
			perc = int(percentage(start, total))
			if file.endswith(".xml") or file.endswith(".hash") or file.endswith("properies"):
				DP.update(perc, "[COLOR %s]Escaneando: [COLOR %s]%s[/COLOR]" % (COLOR2, COLOR1, root.replace(HOME, '')), "[COLOR %s]%s[/COLOR]" % (COLOR1, file), "Espere...[/COLOR]")
...
def wizardUpdate(startup=None):
	if workingURL(WIZARDFILE):
		...
		zip = checkWizard('zip')
		if ver > VERSION:
			yes = DIALOG.yesno(ADDONTITLE, '[COLOR %s]Esta es una nueva version de [COLOR %s]%s[/COLOR]!' % (COLOR2, COLOR1, ADDONTITLE), 'Quieres descargar [COLOR %s]v%s[/COLOR]?[/COLOR]' % (COLOR1, ver), nolabel='[B][COLOR red]Mas tarde[/COLOR][/B]', yeslabel="[B][COLOR green]Actualizar Wizard[/COLOR][/B]")
			if yes:
				log("[Auto Update Wizard] Instalando Wizard v%s" % ver, xbmc.LOGNOTICE)
				DP.create(ADDONTITLE,'[COLOR %s]Descargando Update...' % COLOR2,'', 'Espere...[/COLOR]')
				...
				log("[Auto Update Wizard] Wizard actualizado a v%s" % ver, xbmc.LOGNOTICE)
				reloadProfile()
				return
			else: log("[Auto Update Wizard] Instalacion del Nuevo Wizard Ignorado: %s" % ver, xbmc.LOGNOTICE)
		else: 
			if not startup: LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]No Nueva Version del Wizard[/COLOR]" % COLOR2)
			log("[Auto Update Wizard] No Nueva Version v%s" % ver, xbmc.LOGNOTICE)
	else: log("[Auto Update Wizard] Url del wizard no valido: %s" % WIZARDFILE, xbmc.LOGNOTICE)

def convertText():
	TEXTFILES = os.path.join(ADDONDATA, 'TextFiles')
	if not os.path.exists(TEXTFILES): os.makedirs(TEXTFILES)
	
	DP.create(ADDONTITLE,'[COLOR %s][B]Converting Text:[/B][/COLOR]' % (COLOR2),'', 'Espere...')
	
	if not BUILDFILE == 'http://':
		filename = os.path.join(TEXTFILES, 'builds.txt')
		writing = '';x = 0
		a = openURL(BUILDFILE).replace('\n','').replace('\r','').replace('\t','')
		DP.update(0,'[COLOR %s][B]Converting Text:[/B][/COLOR] [COLOR %s]Builds.txt[/COLOR]' % (COLOR2, COLOR1),'', 'Espere...')
	...
	if not APKFILE == 'http://':
		...
		if len(match2) == 0:
			for name, url, icon, fanart in match:
				...
				writing += 'fanart="%s"\n' % fanart
				writing += 'adult="no"\n'
				writing += 'description="Descargar %s from %s"\n' % (name, ADDONTITLE)
		else:
...
def restoreLocal(type):
	...
	if int(errors) >= 1:
		yes=DIALOG.yesno(ADDONTITLE, '[COLOR %s][COLOR %s]%s[/COLOR]' % (COLOR2, COLOR1, fn), 'Completado: [COLOR %s]%s%s[/COLOR] [Errores:[COLOR %s]%s[/COLOR]]' % (COLOR1, percent, '%', COLOR1, errors), 'Quieres ver los errores?[/COLOR]', nolabel='[B][COLOR red]No Gracias[/COLOR][/B]',yeslabel='[B][COLOR green]Ver Errores[/COLOR][/B]')
		if yes:
...
def restoreExternal(type):
	...
	try: 
		...
		return
	log("[RESTORE EXT BACKUP %s] Archivo: %s " % (type.upper(), source), xbmc.LOGNOTICE)
	zipit = os.path.split(source); zname = zipit[1]
	DP.create(ADDONTITLE,'[COLOR %s]Descargando archivo zip' % COLOR2,'', 'Espere...[/COLOR]')
	if type == "gui": loc = USERDATA
	...
	lookandFeelData('save')
	if int(errors) >= 1:
		yes=DIALOG.yesno(ADDONTITLE, '[COLOR %s][COLOR %s]%s[/COLOR]' % (COLOR2, COLOR1, zname), 'Completed: [COLOR %s]%s%s[/COLOR] [Errors:[COLOR %s]%s[/COLOR]]' % (COLOR1, percent, '%', COLOR1, errors), 'Quieres ver los errores?[/COLOR]', nolabel='[B][COLOR red]No Gracias[/COLOR][/B]',yeslabel='[B][COLOR green]Ver Errores[/COLOR][/B]')
		if yes:
			TextBox(ADDONTITLE, error.replace('\t',''))
	setS('installed', 'true')
	setS('extract', str(percent))
...
def clearThumb(type=None):
	...
	else: choice = DIALOG.yesno("[COLOR=%s]%s[/COLOR]",'[COLOR %s] The thumbnail folder Has exceeded the size of [COLOR red]%s MB[/COLOR] ' % (COLOR1,ADDONTITLE,COLOR4, size), '[COLOR %s]Would you like to delete the [COLOR red]%s[/COLOR] of them?' % (COLOR4, folder), "They will repopulate on the next startup[/COLOR]", nolabel='[B]No Eliminar[/B]', yeslabel='[B]Eliminar Thumbs[/B]')
	if choice == 1:
...
def clearPackagesStart(over=None):
		...
		try:
			for root, dirs, files in os.walk(PACKAGES):
				...
				if file_count > 0:
					...
					if over: yes=1
					else: yes=DIALOG.yesno("[COLOR=%s]%s[/COLOR",'[COLOR %s]La carpeta de paquetes a excedido el volumen en [COLOR red]%s MB[/COLOR] ' % (COLOR1,ADDONTITLE,COLOR4, filesize), "[COLOR %s]%s[/COLOR] archivos encontrados / [COLOR %s]%s[/COLOR] en volumen." % (COLOR1, str(file_count), COLOR3, size), "Quieres eliminarlos?", nolabel='[B]No limpiar[/B]',yeslabel='[B]Limpiar Paquetes[/B]')
					if yes:
						for f in files: os.unlink(os.path.join(root, f))
						for d in dirs: shutil.rmtree(os.path.join(root, d))
						LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE),'[COLOR %s]Limpiar Paquetes: Hecho![/COLOR]' % COLOR2)
				else: LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE),'[COLOR %s]Limpiar Paquetes: No hay![/COLOR]' % COLOR2)
		except Exception, e:
	...
	else: LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE),'[COLOR %s]Limpiando Paquetes: No encontrados![/COLOR]' % COLOR2)
##############################################################################################################
		...
		try:
			for root, dirs, files in os.walk(PACKAGES):
				...
				if file_count > 0:
					size = convertSize(getSize(PACKAGES))
					if over: yes=1
					else: yes=DIALOG.yesno("[COLOR %s]Eliminar Archivos de Paquetes" % COLOR2, "[COLOR %s]%s[/COLOR] archivos encontrados / [COLOR %s]%s[/COLOR] en volumen." % (COLOR1, str(file_count), COLOR1, size), "Quieres eliminarlos?[/COLOR]", nolabel='[B][COLOR red]No limpiar[/COLOR][/B]',yeslabel='[B][COLOR green]Limpiar Paquetes[/COLOR][/B]')
					if yes:
						for f in files: os.unlink(os.path.join(root, f))
						for d in dirs: shutil.rmtree(os.path.join(root, d))
						LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE),'[COLOR %s]Limpiar Paquetes: Hecho![/COLOR]' % COLOR2)
...
def clearPackagesStartup():
	...
	if os.path.exists(PACKAGES):
		...
		try:
			...
			if file_count > 0: LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), '[COLOR %s]Limpiando Paquetes: Hecho: %s[/COLOR]' % (COLOR2, convertSize(cleanupsize)))
			else: LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), '[COLOR %s]Limpiando Paquetes: No encontrados![/COLOR]' % COLOR2)
		except Exception, e:
			LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), '[COLOR %s]Limpiando Paquetes: Error![/COLOR]' % COLOR2)
			log("Clear Packages Error: %s" % str(e), xbmc.LOGERROR)
...
def clearCache(over=None):
				...
				if file_count > 0:
					...
					for d in dirs:
						try:
							shutil.rmtree(os.path.join(root, d))
							delfiles += 1
							log("[Success] limpiado %s archivos de %s" % (str(file_count), os.path.join(item,d)), xbmc.LOGNOTICE)
						except:
	...
	LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), '[COLOR %s]Limpiando Cache: Eliminados %s Archivos[/COLOR]' % (COLOR2, delfiles))
...
def oldThumbs():
	...
	if len(images) > 0: LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), '[COLOR %s]Limpiando Miniaturas: %s Archivos / %s MB[/COLOR]!' % (COLOR2, str(len(images)), removed))
	else: LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), '[COLOR %s]Limpiando Miniaturas: No encontradas![/COLOR]' % COLOR2)
