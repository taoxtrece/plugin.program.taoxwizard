...
from resources.libs import extract, downloader, notify, traktit, net, skinSwitch, uploadLog, yt, speedtest, wizard as wiz, addonwindow as pyxbmct
...
def index():
	...
	addDir ('Herramientas' ,'net', icon=ICONCONTACT, themeit=THEME1)
...
def buildMenu():
	...
	addFile('%s Version: %s' % (MCNAME, KODIV), '', icon=ICONBUILDS, themeit=THEME3)
	addDir ('Save Data Menu'       ,'savedata', icon=ICONSAVE,     themeit=THEME3)
	addDir ('[COLOR purple]---[B][COLOR purple]Addon Packs [COLOR blue]/ [COLOR red]Fixes[/COLOR][/B][COLOR purple]---[/COLOR]'        ,'viewpack',   icon=ICONMAINT,   themeit=THEME1)
	if HIDESPACERS == 'No': addFile(wiz.sep(), '', themeit=THEME3)
	if third == True:
		for item in addin:
...
def viewBuild(name):
	...
	for version, url, url2, url3, gui, kodi, themefile, icon, fanart, adult, description, preview in match:
		...
		if warning == True:
			addFile('BUILD PARA KODI %s [COLOR purple](INSTALLED: %s)[/COLOR]' % (str(kodi), str(KODIV)), '', fanart=fanart, icon=icon, themeit=THEME6)
...
def APKGRAB(name,url):
	html=wiz.openURL(url)
	name=name
	match = re.compile('href="([^"]*)".+?lass="purple_button".+?itle=').findall(html)
	for url in match:
...
def emumenu():
	...
	if len(match) > 0:
		if wiz.platform() == 'android':
			...
			DIALOG.ok(ADDONTITLE, "[COLOR purple]Please go download RetroArch for PC[/COLOR]", " Goto http://tinyurl.com/RetroFTG for a full tutorial")
...
def UNZIPROM():
	...
	if myroms == '':
		if DIALOG.yesno(ADDONTITLE, "[COLOR %s]It seems that you do not have an extract location setup for Rom Packs" % COLOR2, "Would you like to set one?[/COLOR]", yeslabel="[COLOR green][B]Set Location[/B][/COLOR]", nolabel="[COLOR red][B]Cancel Download[/B][/COLOR]"):
			...
			if myroms == '': return
	yes = DIALOG.yesno(ADDONTITLE, "[COLOR %s]Are you sure you would like to download and extract [COLOR %s]%s[/COLOR] to:" % (COLOR2, COLOR1, name), "[COLOR %s]%s[/COLOR]" % (COLOR1, myroms), yeslabel="[B][COLOR green]Download[/COLOR][/B]", nolabel="[B][COLOR red]Cancelar[/COLOR][/B]")
...
def packInstaller(name, url):
	...
	DP.close()
	wiz.LogNotify("[COLOR %s]Addon Installer[/COLOR]" % COLOR1, '[COLOR %s]%s: Instalado![/COLOR]' % (COLOR2, name))
			...
			if len(match) > 0:
				for name, url, repository, repositoryxml, repositoryurl, icon, fanart, adult, description in match:
					if os.path.exists(os.path.join(ADDONS, plugin)):
						...
						selected = DIALOG.select("[COLOR %s]Addon instalado que quieres hacer?[/COLOR]" % COLOR2, do)
						if selected == 0:
							...
							if DIALOG.yesno(ADDONTITLE, "[COLOR %s]Would you like to remove the addon_data for:" % COLOR2, "[COLOR %s]%s[/COLOR]?[/COLOR]" % (COLOR1, plugin), yeslabel="[B][COLOR green]Si eliminar[/COLOR][/B]", nolabel="[B][COLOR red]No omitir[/COLOR][/B]"):
					...
					if not repository.lower() == 'none' and not os.path.exists(repo):
						...
						if DIALOG.yesno(ADDONTITLE, "[COLOR %s]Would you like to install the repository for [COLOR %s]%s[/COLOR]:" % (COLOR2, COLOR1, plugin), "[COLOR %s]%s[/COLOR]?[/COLOR]" % (COLOR1, repository), yeslabel="[B][COLOR green]Si, Instalar[/COLOR][/B]", nolabel="[B][COLOR red]No, Omitir[/COLOR][/B]"): 
...
def writeAdvanced(name, url):
	...
	if ADVANCEDWORKING == True:
		if os.path.exists(ADVANCED): choice = DIALOG.yesno(ADDONTITLE, "[COLOR %s]Would you like to overwrite your current Advanced Settings with [COLOR %s]%s[/COLOR]?[/COLOR]" % (COLOR2, COLOR1, name), yeslabel="[B][COLOR green]Overwrite[/COLOR][/B]", nolabel="[B][COLOR red]Cancelar[/COLOR][/B]")
...
def removeAddonMenu():
	...
	if len(selected) > 0:
		...
		else: todo = DIALOG.yesno(ADDONTITLE, "[COLOR %s]Prefieres [COLOR %s]Forzar Cierre[/COLOR] de kodi o[COLOR %s]Recargar Perfil[/COLOR]?[/COLOR]" % (COLOR2, COLOR1, COLOR1), yeslabel="[B][COLOR red]Recargar Perfil[/COLOR][/B]", nolabel="[B][COLOR green]Forzar Cierre[/COLOR][/B]")
...
def removeAddonDataMenu():
	if os.path.exists(ADDOND):
		...
		for folder in sorted(fold, key = lambda x: x):
			...
			replace = {'audio.':'[COLOR orange][AUDIO] [/COLOR]', 'metadata.':'[COLOR cyan][METADATA] [/COLOR]', 'module.':'[COLOR orange][MODULE] [/COLOR]', 'plugin.':'[COLOR blue][PLUGIN] [/COLOR]', 'program.':'[COLOR orange][PROGRAM] [/COLOR]', 'repository.':'[COLOR gold][REPO] [/COLOR]', 'script.':'[COLOR green][SCRIPT] [/COLOR]', 'service.':'[COLOR green][SERVICE] [/COLOR]', 'skin.':'[COLOR dodgerblue][SKIN] [/COLOR]', 'video.':'[COLOR orange][VIDEO] [/COLOR]', 'weather.':'[COLOR purple][WEATHER] [/COLOR]'}
			for rep in replace:
...
def buildWizard(name, type, theme=None, over=False):
	...
	if type == 'gui':
		if name == BUILDNAME:
			if over == True: yes = 1
			else: yes = DIALOG.yesno(ADDONTITLE, '[COLOR %s]Would you like to apply the guifix for:' % COLOR2, '[COLOR %s]%s[/COLOR]?[/COLOR]' % (COLOR1, name), nolabel='[B][COLOR red]No, Cancelar[/COLOR][/B]',yeslabel='[B][COLOR green]Apply Fix[/COLOR][/B]')
		else: 
			yes = DIALOG.yesno("%s - [COLOR red]WARNING!![/COLOR]" % ADDONTITLE, "[COLOR %s][COLOR %s]%s[/COLOR] community build is not currently installed." % (COLOR2, COLOR1, name), "Would you like to apply the guiFix anyways?.[/COLOR]", nolabel='[B][COLOR red]No, Cancelar[/COLOR][/B]',yeslabel='[B][COLOR green]Apply Fix[/COLOR][/B]')
		if yes:
			...
			if not os.path.exists(PACKAGES): os.makedirs(PACKAGES)
			DP.create(ADDONTITLE,'[COLOR %s][B]Downloading GuiFix:[/B][/COLOR] [COLOR %s]%s[/COLOR]' % (COLOR2, COLOR1, name),'', 'Espere...')
		...
		if yes_pressed:
			...
			if int(float(percent)) > 0:
				...
				DIALOG.ok(ADDONTITLE, "[COLOR %s]Para guardar los cambios se necesita forzar el cierre, Presione OK para forzar el cierre[/COLOR]" % COLOR2); wiz.killxbmc('true')
			else:
				if isinstance(errors, unicode):
					error = error.encode('utf-8')
		...
		if theme == None:
			...
			if not themefile == 'http://' and wiz.workingURL(themefile) == True:
				...
				if len(themelist) > 0:
					if DIALOG.yesno(ADDONTITLE, "[COLOR %s]The Build [COLOR %s]%s[/COLOR] comes with [COLOR %s]%s[/COLOR] different themes" % (COLOR2, COLOR1, name, COLOR1, len(themelist)), "Would you like to install one now?[/COLOR]", yeslabel="[B][COLOR green]Instalar Theme[/COLOR][/B]", nolabel="[B][COLOR red]Cancel Themes[/COLOR][/B]"):
		...
		else: installtheme = DIALOG.yesno(ADDONTITLE, '[COLOR %s]Would you like to install the theme:' % COLOR2, '[COLOR %s]%s[/COLOR]' % (COLOR1, theme), 'for [COLOR %s]%s v%s[/COLOR]?[/COLOR]' % (COLOR1, name, wiz.checkBuild(name,'version')), yeslabel="[B][COLOR green]Instalar Theme[/COLOR][/B]", nolabel="[B][COLOR red]Cancel Themes[/COLOR][/B]")
		if installtheme:
...
def apkInstaller1(apk, url):
	if wiz.platform() == 'android' or DEVELOPER == 'true':
		...
		if yes:
			...
			DP.create(ADDONTITLE,'Descargando... %s' % display,'', 'Espere...')
			lib=os.path.join(PACKAGES, "%s.apk" % apk.replace('\\', '').replace('/', '').replace(':', '').replace('*', '').replace('?', '').replace('"', '').replace('<', '').replace('>', '').replace('|', ''))
			try: os.remove(lib)
			except: pass
			downloader.download(url, lib, DP)
			xbmc.sleep(500)
			DP.close()
			DIALOG.ok(ADDONTITLE, "Lanzar APK para instalar", "Continua el proceso de instalacion.")
			xbmc.executebuiltin('StartAndroidActivity("","android.intent.action.VIEW","application/vnd.android.package-archive","file:'+lib+'")')
		else: wiz.LogNotify(ADDONTITLE, '[COLOR red]ERROR:[/COLOR] Instalacion cancelada')
	else: wiz.LogNotify(ADDONTITLE, '[COLOR red]ERROR:[/COLOR] No es un dispositivo Android')
...
def toggleCache(state):
	cachelist = ['includevideo', 'includeall', 'includeelementum', 'includealfa,' 'includebalandro']
	titlelist = ['Include Video Addons', 'Include All Addons', 'Include Elementum', 'Include Alfa', 'Include Balandro']
	...
	if wiz.workingURL(BUILDFILE) == True:
		if wiz.checkBuild(name, 'url'):
			...
			if extend == True:
				...
				msg += "[COLOR %s]Extrayendo:[/COLOR] [COLOR %s]%s[/COLOR][CR][CR]" % (COLOR2, COLOR1, wiz.convertSize(int(float(extracted))))
				msg += "[COLOR %s]Zip Descargado:[/COLOR] [COLOR %s]%s[/COLOR][CR][CR]" % (COLOR2, COLOR1, wiz.convertSize(int(float(zipsize))))
				msg += "[COLOR %s]Skin Nombre:[/COLOR] [COLOR %s]%s[/COLOR][CR][CR]" % (COLOR2, COLOR1, skin)
				msg += "[COLOR %s]Adult Content:[/COLOR] [COLOR %s]%s[/COLOR][CR][CR]" % (COLOR2, COLOR1, adult)
				msg += "[COLOR %s]Descripcion:[/COLOR] [COLOR %s]%s[/COLOR][CR][CR]" % (COLOR2, COLOR1, description)
				msg += "[COLOR %s]Programas:[/COLOR] [COLOR %s]%s[/COLOR][CR][CR]" % (COLOR2, COLOR1, programs)
				...
				msg += "[COLOR %s]Repositorios:[/COLOR] [COLOR %s]%s[/COLOR][CR][CR]" % (COLOR2, COLOR1, repos)
				msg += "[COLOR %s]Scripts:[/COLOR] [COLOR %s]%s[/COLOR]" % (COLOR2, COLOR1, scripts)
			else:
				msg  = "[COLOR %s]Build:[/COLOR] [COLOR %s]%s[/COLOR][CR]" % (COLOR2, COLOR1, name)
				msg += "[COLOR %s]Version:[/COLOR] [COLOR %s]%s[/COLOR][CR]" % (COLOR2, COLOR1, version)
				if not theme == "http://":
					themecount = wiz.themeCount(name, False)
					msg += "[COLOR %s]Build Tema(s):[/COLOR] [COLOR %s]%s[/COLOR][CR]" % (COLOR2, COLOR1, ', '.join(themecount))
...
def addonpackwiz():
		yes = DIALOG.yesno(ADDONTITLE, '[COLOR %s]Quiere instalar:' % COLOR2, '[COLOR %s]%s[/COLOR]?[/COLOR]' % (COLOR1, name), nolabel='[B]No, Cancelar[/B]',yeslabel='[B]Instalar[/B]')
		if yes:
			if not os.path.exists(PACKAGES): os.makedirs(PACKAGES)
			DP.create(ADDONTITLE,'[COLOR %s][B]Descargando:[/B][/COLOR] [COLOR %s]%s[/COLOR]' % (COLOR2, COLOR1, name),'', 'Espere...')
			...
			yes = DIALOG.yesno(ADDONTITLE, '[COLOR %s]Instalado:' % COLOR2, '[COLOR %s]%s[/COLOR]?[/COLOR]' % (COLOR1, name),'[COLOR green]Satisfactorio[/COLOR]', nolabel='[B]Reset[/B]',yeslabel='[B]Force Close[/B]')
			if yes == 1:
				wiz.killxbmc('true')
			elif yes == 0:
				wiz.RESET()
...
def manageSaveData(do):
	if do == 'import':
		...
		source = DIALOG.browse(1, '[COLOR %s]Seleccione destino de SaveData.zip[/COLOR]' % COLOR2, 'files', '.zip', False, False, HOME)
		if not source.endswith('.zip'):
			wiz.LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]Importar Data Error![/COLOR]" % (COLOR2))
			return
		...
		if os.path.exists(trakt):
			...
			for item in files:
				...
				if os.path.exists(old):
					if not DIALOG.yesno(ADDONTITLE, "[COLOR %s]Quiere reemplazar el actual [COLOR %s]%s[/COLOR] archivo?" % (COLOR2, COLOR1, item), yeslabel="[B][COLOR green]Si Reemplazar[/COLOR][/B]", nolabel="[B][COLOR red]No, Omitir[/COLOR][/B]"): continue
					else: os.remove(old)
		...
		dir = [traktit.TRAKTFOLD,]
		traktit.traktIt('update', 'all')
		source = DIALOG.browse(3, '[COLOR %s]Donde desea exportar el savedata zip?[/COLOR]' % COLOR2, 'files', '', False, True, HOME)
...
def freshStart(install=None, over=False):
	...
	elif install == 'restore': yes_pressed=DIALOG.yesno(ADDONTITLE, "[COLOR %s]Presiona [B][COLOR springgreen] Si [/COLOR][/B]" % COLOR2, "Para eliminar el build actual, \r\nY restaura backup del build local o externo[/COLOR]", nolabel='[B][COLOR red]No, Cancelar[/COLOR][/B]', yeslabel='[B][COLOR springgreen]Si[/COLOR][/B]')
	elif install: yes_pressed=DIALOG.yesno(ADDONTITLE, "[COLOR %s]Presiona [B][COLOR springgreen] Si [/COLOR][/B]" % COLOR2, "Para eliminar el build actual, \r\ne Instalar [COLOR %s]%s[/COLOR]!!" % (COLOR1, install), nolabel='[B][COLOR red]No, Cancelar[/COLOR][/B]', yeslabel='[B][COLOR springgreen]Si[/COLOR][/B]')
	else: yes_pressed=DIALOG.yesno(ADDONTITLE, "[COLOR %s]Quieres restablecer" % COLOR2, "la configuracion a por defecto?[/COLOR]", nolabel='[B][COLOR red]No, Cancelar[/COLOR][/B]', yeslabel='[B][COLOR springgreen]Si[/COLOR][/B]')
	if yes_pressed:
		if not wiz.currSkin() in ['skin.confluence', 'skin.estuary']:
			...
			for name in files:
				...
				if name == 'sources.xml' and fold[-1] == 'userdata' and KEEPSOURCES == 'true': wiz.log("Keep Sources: %s" % os.path.join(root, name), xbmc.LOGNOTICE)
					...
					try: os.remove(os.path.join(root,name))
						...
						wiz.log("Error eliminando %s" % os.path.join(root, name), xbmc.LOGNOTICE)
						wiz.log("-> / %s" % (str(e)), xbmc.LOGNOTICE)
			if DP.iscanceled(): 
				DP.close()
		...
		for root, dirs, files in os.walk(xbmcPath,topdown=True):
			dirs[:] = [d for d in dirs if d not in EXCLUDES]
			for name in dirs:
				DP.update(100, '', 'Limpiando carpetas: [COLOR %s]%s[/COLOR]' % (COLOR1, name), '')
		...
		if over == True:
			...
			else: todo = DIALOG.yesno(ADDONTITLE, "[COLOR %s]Necesita [COLOR %s]Forzar Cierre[/COLOR] Esta aplicacion[COLOR %s]And[/COLOR] Para volver abrir[/COLOR]" % (COLOR2, COLOR1, COLOR1), yeslabel="[B][COLOR white]>>>>>>[/COLOR][/B]", nolabel="[B][COLOR springgreen]Forzar Cierre[/COLOR][/B]")
...
def clearCache():
	if DIALOG.yesno(ADDONTITLE, '[COLOR %s]Quieres limpiar cache?[/COLOR]' % COLOR2, nolabel='[B][COLOR red]No, Cancelar[/COLOR][/B]', yeslabel='[B][COLOR green]Limpiar cache[/COLOR][/B]'):
		wiz.clearCache()
		DC.setLabel('Descargado: [B][COLOR purple]0.0 B[/B][/COLOR]')
def clearArchive():
	if DIALOG.yesno(ADDONTITLE, '[COLOR %s]Quieres limpiar \'Archive_Cache\' carpeta?[/COLOR]' % COLOR2, nolabel='[B][COLOR red]No, Cancelar[/COLOR][/B]', yeslabel='[B][COLOR green]Si, Limpiar[/COLOR][/B]'):
		wiz.clearArchive()
def clearPackages():
	if DIALOG.yesno(ADDONTITLE, '[COLOR %s]Quieres eliminar paquetes?[/COLOR]' % COLOR2, nolabel='[B][COLOR red]No, Cancelar[/COLOR][/B]', yeslabel='[B][COLOR green]Eliminar[/COLOR][/B]'):
		wiz.clearPackages('total')
		DPK.setLabel('Descargado: [B][COLOR purple]0.0 B[/B][/COLOR]')
		TPK.setLabel('Archivos: [B][COLOR purple]0[/B][/COLOR]')

def totalClean():
	if DIALOG.yesno(ADDONTITLE, '[COLOR %s]Quieres limpiar cache, packages y thumbnails?[/COLOR]' % COLOR2, nolabel='[B][COLOR red]Cancelar Proceso[/COLOR][/B]',yeslabel='[B][COLOR green]Limpiar Todo[/COLOR][/B]'):
		wiz.clearCache()
		wiz.clearPackages('total')
		clearThumb('total')
		TC.setLabel('Descargado: [B][COLOR purple]0.0 B[/B][/COLOR]')
		DC.setLabel('Descargado: [B][COLOR purple]0.0 B[/B][/COLOR]')
		DPK.setLabel('Descargado: [B][COLOR purple]0.0 B[/B][/COLOR]')
		TPK.setLabel('Archivos: [B][COLOR purple]0[/B][/COLOR]')
		DTH.setLabel('Descargado: [B][COLOR purple]0.0 B[/B][/COLOR]')
		TTH.setLabel('Archivos: [B][COLOR purple]0[/B][/COLOR]')
def clearThumb(type=None):
	latest = wiz.latestDB('Textures')
	if not type == None: choice = 1
	else: choice = DIALOG.yesno(ADDONTITLE, '[COLOR %s]Quieres eliminar %s y la carpeta Thumbnails?' % (COLOR2, latest), "Se recargaran en el proximo inicio[/COLOR]", nolabel='[B][COLOR red]No, Eliminar[/COLOR][/B]', yeslabel='[B][COLOR green]Eliminar Thumbs[/COLOR][/B]')
	if choice == 1:
...
def list_update():
	...
	try:
		if window.getFocus() == buildlist:
			...
			buildtextbox.setLabel('[COLOR %s]Build Seleccionada: [COLOR %s]%s[/COLOR]' % (DES_T_COLOR,DESCOLOR, name))
			...
			desctextbox.setText('[COLOR %s]Build Descripcion: [COLOR %s]%s[/COLOR]' % (DES_T_COLOR,DESCOLOR,description))
	...
	try:
		if window.getFocus() == addonlist:
			...
			addtextbox.setLabel('[COLOR %s]Addon Seleccionado:[COLOR %s] %s[/COLOR]' % (DES_T_COLOR,DESCOLOR,name))
			desctextboxA.setText('[COLOR %s]Addon Descripcion:[COLOR %s] %s[/COLOR]' % (DES_T_COLOR,DESCOLOR,description))
	...
	try:
		if window.getFocus() == apklist:
			pos=apklist.getSelectedPosition()
			link = net.http_GET(APKFILE).content.replace('\n','').replace('\r','')
			apkpic = re.compile('icon="(.+?)"').findall(link)[pos]
			url = re.compile('url="(.+?)"').findall(link)[pos]
			name = re.compile('name="(.+?)"').findall(link)[pos]
			description = re.compile('description="(.+?)"').findall(link)[pos]
			apktextbox.setLabel('[COLOR %s]APK Seleccionada:[COLOR %s] %s[/COLOR]' % (DES_T_COLOR,DESCOLOR,name))
			desctextboxAPK.setText('[COLOR %s]APK Descripcion:[COLOR %s] %s[/COLOR]' % (DES_T_COLOR,DESCOLOR,description))
	...
	try:
		if window.getFocus() == emulist:
			...
			name = re.compile('name="(.+?)"').findall(link)[pos]
			description = re.compile('description="(.+?)"').findall(link)[pos]
			emutextbox.setLabel('[COLOR %s]EMU Seleccionado:[COLOR %s] %s[/COLOR]' % (DES_T_COLOR,DESCOLOR,name))
			desctextboxEMU.setText('[COLOR %s]EMU Descripcion:[COLOR %s] %s[/COLOR]' % (DES_T_COLOR,DESCOLOR,description))
	...
	try:
		if window.getFocus() == romlist:
			...
			name = re.compile('name="(.+?)"').findall(link)[pos]
			description = re.compile('description="(.+?)"').findall(link)[pos]
			romtextbox.setLabel('[COLOR %s]ROM PACK Seleccionado:[COLOR %s] %s[/COLOR]' % (DES_T_COLOR,DESCOLOR,name))
			desctextboxROM.setText('[COLOR %s]ROM PACK Descripcion:[COLOR %s] %s[/COLOR]' % (DES_T_COLOR,DESCOLOR,description))
...
def BuildList():
	...
	if not BUILDFILE == 'http://' and not BUILDFILE == '':
		...
		FreshStartButton = pyxbmct.Button('[COLOR %s][B]INSTALAR BUILD[/B][/COLOR]' % OTHER_BUTTONS_TEXT,focusTexture=FBUTTON,noFocusTexture=BUTTON)
		window.placeControl(FreshStartButton,30 , 19, 10, 10)
		window.connect(FreshStartButton,lambda: buildWizard(Bname,'fresh'))
		
		InstallButton = pyxbmct.Button('[COLOR %s][B]INSTALAR PARCHE[/B][/COLOR]' % OTHER_BUTTONS_TEXT,focusTexture=FBUTTON,noFocusTexture=BUTTON)
		window.placeControl(InstallButton,44 , 19, 10, 10)
		...
		buildname  = pyxbmct.Label('[COLOR %s]Build Actual: [COLOR %s]%s[/COLOR]' % (DES_T_COLOR,DESCOLOR,buildname))
		window.placeControl(buildname, 90, 2, 8, 25)
		
		buildversion  = pyxbmct.Label('[COLOR %s]Version Build Actual: [COLOR %s]%s[/COLOR]' % (DES_T_COLOR,DESCOLOR,buildversion))
		window.placeControl(buildversion, 95, 2, 8, 20)
		
		errorinstall  = pyxbmct.Label('[COLOR %s]Errores durante la instalacion: [COLOR %s]%s[/COLOR]' % (DES_T_COLOR,DESCOLOR,errors))
		window.placeControl(errorinstall, 95, 28, 8, 20)
		
		lastupdatchk  = pyxbmct.Label('[COLOR %s]Ultimo Update: [COLOR %s]%s[/COLOR]' % (DES_T_COLOR,DESCOLOR,lastbuildcheck))
		window.placeControl(lastupdatchk, 90, 28, 8, 21)
		...
		buildlist.controlRight(FreshStartButton)
		buildlist.controlUp(BuildsButton)
		
		FreshStartButton.controlDown(InstallButton)
		FreshStartButton.controlUp(BuildsButton)
		FreshStartButton.controlLeft(buildlist)
		
		InstallButton.controlDown(FreshStartButton)
		InstallButton.controlUp(FreshStartButton)
...
def Maint():
	...
	#maint lables#
	TC = pyxbmct.Label('[COLOR %s]Descargado:[/COLOR] [COLOR %s]%s[/COLOR]' % (DES_T_COLOR, DESCOLOR,wiz.convertSize(totalsize)))
	window.placeControl(TC, 80 , 21 ,  10, 9)
	DC = pyxbmct.Label('[COLOR %s]Descargado:[/COLOR] [COLOR %s]%s[/COLOR]' % (DES_T_COLOR, DESCOLOR,wiz.convertSize(sizecache)))
	window.placeControl(DC, 96 , 21 ,  10, 9)
	DPK = pyxbmct.Label('[COLOR %s]Descargado:[/COLOR] [COLOR %s]%s[/COLOR]' % (DES_T_COLOR, DESCOLOR,wiz.convertSize(sizepack)))
	window.placeControl(DPK, 96 , 31 ,  10, 9)
	TPK = pyxbmct.Label('[COLOR %s]Archivos:[/COLOR] [COLOR %s]%s[/COLOR]' % (DES_T_COLOR, DESCOLOR,totalpack))
	window.placeControl(TPK, 101 , 31 ,  10, 9)
	DTH = pyxbmct.Label('[COLOR %s]Descargado:[/COLOR] [COLOR %s]%s[/COLOR]' % (DES_T_COLOR, DESCOLOR,wiz.convertSize(sizethumb)))
	window.placeControl(DTH, 96 , 41 ,  10, 9)
	TTH = pyxbmct.Label('[COLOR %s]Archivos:[/COLOR] [COLOR %s]%s[/COLOR]' % (DES_T_COLOR, DESCOLOR,totalthumb))
...
def Installables():
	...
	APKButton = pyxbmct.Button('[COLOR %s][B]Android[/B][/COLOR]' % OTHER_BUTTONS_TEXT,focusTexture=EXIT,noFocusTexture=BUTTON)
...
MaintButton = pyxbmct.Button('[COLOR %s][B]Mantenimiento[/B][/COLOR]' % MAIN_BUTTONS_TEXT,focusTexture=FBUTTON,noFocusTexture=BUTTON)
...
ToolsButton = pyxbmct.Button('[COLOR %s][B]Herramientas[/B][/COLOR]' % MAIN_BUTTONS_TEXT,focusTexture=FBUTTON,noFocusTexture=BUTTON)
window.placeControl(ToolsButton,2 , 25, 9, 8)
window.connect(ToolsButton, lambda: Tools())

InstallablesButton = pyxbmct.Button('[COLOR %s][B]Instalables[/B][/COLOR]' % MAIN_BUTTONS_TEXT,focusTexture=FBUTTON,noFocusTexture=BUTTON)
window.placeControl(InstallablesButton,2 , 33, 9, 8)
window.connect(InstallablesButton, lambda: Installables())

CloseButton = pyxbmct.Button('[COLOR %s][B]Cerrar[/B][/COLOR]' % MAIN_BUTTONS_TEXT,focusTexture=EXIT,noFocusTexture=BUTTON)
